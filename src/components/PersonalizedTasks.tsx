...
import { fetchTasksFromGemini } from '@/services/gemini';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Clock, X, ArrowLeft, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        setLoading(true);
        setError(null);
        const apiTasks = await fetchTasksFromGemini(subject);
        setTasks(apiTasks);
      } catch (err) {
      ...
      <Card className="card-modern">
        <CardHeader>
          <CardTitle className="text-2xl">
            {subject ? `${subject.charAt(0).toUpperCase() + subject.slice(1)} Study Tasks` : 'Personalized Tasks'}
          </CardTitle>
            ...
            tasks.map((task, index) => (
              ...
              >
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setSelectedTask(task)} // only Start button triggers popup
                >
                  Start
                </Button>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full animate-fade-in">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm ${getPriorityColor(selectedTask.priority)} px-2 py-0.5 rounded-full text-white`}>
